...
local function ApplyFaderConfig(frame, faderConfig)
  if not frame then
    return
  end

  frame.faderConfig = faderConfig or frame.faderConfig or {}
end

function L:StartFadeIn(frame)
  if not (frame and frame.fader and frame.faderConfig) then return end
  if frame.faderConfig.enabled == false then
    frame.fader:Pause()
    frame.fader.direction = nil
...
local function FrameHandler(frame)
  ...
  local config = frame.faderConfig
  ...
  if config.enabled == false then
    ...
    frame:SetAlpha(config.fadeInAlpha or 1)
    return
  end
...
function rLib:CreateFrameFader(frame, faderConfig)
  ...
  if not frame.__faderHooksInstalled then
    frame:HookScript("OnEnter", FrameHandler)
    frame:HookScript("OnLeave", FrameHandler)
    frame.__faderHooksInstalled = true
  end
  FrameHandler(frame)
end

local function BuildButtonBarFaderConfig(fadeIn, fadeOut, enabled)
  return {
    enabled = enabled ~= false,
    fadeInAlpha = fadeIn and fadeIn.alpha or 1,
    fadeInDuration = fadeIn and fadeIn.time or 0.3,
    fadeInDelay = 0,
    fadeInSmooth = "OUT",
    fadeOutAlpha = fadeOut and fadeOut.alpha or 0,
    fadeOutDuration = fadeOut and fadeOut.time or 0.3,
    fadeOutDelay = fadeOut and fadeOut.delay or 0,
    fadeOutSmooth = "OUT",
  }
end
...
function rButtonBarFader(frame,buttonList,fadeIn,fadeOut)
  ...
  local faderConfig = BuildButtonBarFaderConfig(fadeIn, fadeOut, true)
  rLib:CreateButtonFrameFader(frame, buttonList, faderConfig)
end

function rButtonBarFaderUpdate(frame, fadeIn, fadeOut, enabled)
  if not frame then
    return false
  end

  local buttonList = frame.__buttonBarFaderButtons
  if buttonList and not frame.fader then
    local faderConfig = BuildButtonBarFaderConfig(fadeIn, fadeOut, enabled)
    rLib:CreateButtonFrameFader(frame, buttonList, faderConfig)
    return true
  end

  if not frame.faderConfig then
  ...
  ApplyFaderConfig(frame, BuildButtonBarFaderConfig(fadeIn, fadeOut, enabled))
  FrameHandler(frame)
  return true
end
